...
const getCollectionName = (format: any, rawData: any) => {
...
const convertCollection = async (format: any, rawData: any, groupingType: any) => {
  try {
    let collection;

    switch (format) {
      case 'openapi':
        collection = await convertOpenapiToBruno(rawData, { groupBy: groupingType });
        break;
      case 'wsdl':
      ...
      case 'postman':
        ...
        break;
      case 'insomnia':
        collection = await convertInsomniaToBruno(rawData);
        break;
      case 'bruno':
...
const ImportCollectionLocation = ({
  onClose,
  handleSubmit,
  rawData,
  format
  ...
  const activeWorkspace = workspaces.find((w: any) => w.uid === activeWorkspaceUid) as any;
